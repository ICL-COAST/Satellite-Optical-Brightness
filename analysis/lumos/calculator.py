...
def sun_and_obs_dirs_in_sat_frame(
    sat_alt_deg, sat_az_deg,
    sun_alt_deg, sun_az_deg,
    sat_height_km=550,
):
    sat_height_m = sat_height_km * 1000.0

    # 1) Convert from HCS (observer Alt/Az) to brightness frame coordinates
    ...
    # 2) Observer direction vector (satellite -> observer), normalized
    v = np.array([obs_x, obs_y, obs_z], dtype=float)
    o_hat = v / np.linalg.norm(v)

    # 3) Sun direction vector in the satellite (brightness) frame, unit vector
    s_hat = np.array([0.0,
                      np.cos(angle_past_terminator),
                      -np.sin(angle_past_terminator)],
                     dtype=float)
